...
import { ArrowLeft, Quote, Heart, Sparkles, Feather, Download, Lock } from "lucide-react";
import { onAuthStateChanged, signInWithPopup, User } from "firebase/auth";
import { doc, getDoc, serverTimestamp, setDoc } from "firebase/firestore";
import { auth, db, googleProvider } from "@/lib/firebase";

export default function KhomsatsatInfoClient() {
  ...
  const renderStartButton = (label: string) => {
    if (authLoading || profileLoading) {
      return <div className="mx-auto h-[60px] w-[260px] animate-pulse rounded-full bg-fuchsia-100" />;
    }
    ...
    if (!user) {
      return (
        <button
          ...
          className="mx-auto flex items-center gap-3 rounded-full border border-slate-200 bg-white px-8 py-4 text-lg font-black text-slate-900 shadow-xl transition-all hover:-translate-y-1 hover:bg-slate-50 disabled:opacity-70"
        >
          <img loading="lazy" decoding="async" src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="Google" className="h-6 w-6" />
          {isLoggingIn ? "กำลังเข้าสู่ระบบ..." : "เข้าสู่ระบบก่อนใช้งาน"}
        </button>
      );
    }
    ...
    if (!isKhomsatsatUnlocked) {
      return (
        <button
          disabled
          className="mx-auto flex cursor-not-allowed items-center gap-3 rounded-full border border-fuchsia-100 bg-white px-8 py-4 text-base font-black text-slate-400 shadow-sm"
        >
          <Lock size={20} />
          ล็อกอยู่ ต้องจบ Phase 1 ก่อน
        </button>
      );
    }
    ...
    <div className="min-h-screen bg-fuchsia-50/50 text-slate-800 font-sans pb-20">
      ...
      <main className="max-w-3xl mx-auto px-4 mt-8">
        ...
        <header className="text-center mb-12">
          ...
          {renderStartButton("เริ่มสร้างคำคมของคุณ")}
          <p className="text-slate-400 text-[12px] mt-3 font-medium">
            {user && !isKhomsatsatUnlocked ? "ทำ Phase 1 ใน Dashboard ให้ครบก่อน แล้วค่อยกลับมาเปิดคมสัดสัด" : "เพื่อเก็บข้อมูล แนะนำให้ Login ผ่าน Gmail ที่หน้าแรกก่อน"}
          </p>
        </header>
