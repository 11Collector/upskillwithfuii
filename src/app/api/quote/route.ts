  ...
  try {
    const response = await fetch("https://api.deepseek.com/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { 
            ...
            content: type === "second_brain_scan"
              ? "คุณคือ AI ผู้ช่วยสแกนระบบความคิด ทำหน้าที่วิเคราะห์ความเชื่อมโยงเชิงแนวคิดซ่อนเร้นระหว่างหัวข้อและเนื้อหาของโน้ตต่างๆ\n\nตอบกลับในรูปแบบ JSON Object เท่านั้น มีโครงสร้างดังนี้:\n{\n  \"connections\": [\n    {\n      \"source\": \"[ID ของโน้ต A]\",\n      \"target\": \"[ID ของโน้ต B]\",\n      \"score\": 85,\n      \"reason\": \"[คำอธิบายจุดเชื่อมโยงซ่อนเร้น 1 ประโยค ห้ามใช้เครื่องหมายอัญประกาศคู่ซ้ำในข้อความ]\"\n    }\n  ]\n}\nค้นหาและจับคู่ทุกคู่โน้ตที่มีความเชื่อมโยงเชิงแนวคิดกันอย่างมีนัยสำคัญ โดยประเมินคะแนนความสัมพันธ์ (score) ระหว่าง 75-100% (ส่งมาได้กี่คู่ก็ได้ตามความสัมพันธ์จริงของข้อมูล โดยเลือกเฉพาะคู่ที่มีความเชื่อมโยงเกิน 75% ขึ้นไปเท่านั้น หากไม่พบความเชื่อมโยง ให้ส่ง connections เป็น Array ว่าง [])"
              : type === "second_brain"
        ...
        max_tokens: type === "wheel" ? 2500 : type === "second_brain_scan" ? 2500 : type === "second_brain" ? 1000 : 1200
      })
    });
